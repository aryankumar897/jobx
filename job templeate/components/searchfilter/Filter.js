...
import { priceRanges } from "@/utils/salaryData";
...
export default function Filter({ searchParams }) {
    const pathname = "/jobs";
    const { minSalary, maxSalary, countryid, stateid, cityid, tagid, jobcateid } = searchParams;


    const { fetchTagPublic, tag } = useTag();
            ...
            <div className="mb-5 overflow-scroll   filter-scrolls">
                ...
                <div className="row d-flex align-items-center mx-1">
                    {priceRanges?.map((range) => {
                        ...
                        const isActive =
                            minSalary === String(range?.min) && maxSalary === String(range?.max);
                        return (
                            <div key={range?.label}>
                                <Link href={url}
                ...
                <div className="row d-flex align-items-center mx-1 filter-scroll">
                    {job_categories?.map((c) => {
                        const isActive = jobcateid === c._id;
                        ...
                        return (
                            <div key={c._id}>
                                ...
                                {isActive && (
                                    <span
                                        onClick={() => handleRemoveFilter("jobcateid")}
                                        className="pointer"
                                    >
                                        X
                                    </span>
